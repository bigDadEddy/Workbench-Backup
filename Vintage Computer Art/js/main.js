...
function anim() {
	...
	var r = Math.round(lerp(0, 100, Math.sin(.3 * c)/2 + 1));
	    g = Math.round(lerp(0, 155, Math.sin(.4 * c)/2 + 1));
	    b = Math.round(lerp(100, 255, Math.sin(.5 * c)/2 + 1));
    ...
    for (var i = s; i < 360 + s; i += n) {
		//l = ox * 3 / 4 + ox * 1 / 4 * Math.sin(degToRad(.7 * i + s));
        x = ox + l * Math.cos(degToRad(1 * i)); 
        y = oy + l * Math.sin(degToRad(1 * i));
		var temp = i % 2 == 0 ? colorR : colorB;
		console.log(temp);
        var line = createLine(ox, oy, x, y, temp);		 
